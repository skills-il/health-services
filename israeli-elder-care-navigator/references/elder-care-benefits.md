# Israeli Elder Care Benefits Reference (2026)

## Old-Age Pension (Kiztavat Zikna) -- Bituach Leumi

### Retirement Age (2026)
- Men: 67
- Women: 63 in 2026 (for women born 1962). Phases up by birth year: 63y 3m (born 1963), 63y 6m (1964), 63y 9m (1965), 64 (1966), 64y 3m (1967), 64y 6m (1968), 64y 9m (1969), 65 (1970+).

### Monthly Amounts (January 2026)
- Basic individual pension (up to 80): 1,838 NIS
- Individual pension age 80+: 1,941 NIS (includes 103 NIS age supplement)
- Couple base (one earner): 2,762 NIS
- Couple, both aged 80+: 2,865 NIS
- Spouse supplement: 924 NIS
- Child supplement: 581 NIS (first 2 children only)
- Seniority supplement (vatik): 2% per insured year, paid for up to 25 years, max 50% of the basic pension
- Health insurance deduction: -237 NIS (individual), -340 NIS (couple)

### Income Supplement Eligibility (2026)
Ceilings are per income type, not one combined household figure:

| Income source | Individual | Couple |
|---|---|---|
| BL pension only (old-age and/or survivors) | 4,375 NIS | 6,912 NIS |
| From work | 3,236 NIS | 3,786 NIS |
| From an occupational pension | 1,790 NIS | 2,823 NIS |

Above the work ceiling, 60% of the excess is deducted. Kibbutz and moshav shitufi members are not eligible.

### Income Test
- Under 70: pension is means-tested (income from work reduces pension)
- 70+: everyone receives pension regardless of income
- Income supplement (hashlamat hachnasa) available for low-income elderly

## Long-Term Care Benefit (Gimlat Siud) -- Bituach Leumi

### Eligibility
- Reached retirement age
- Lives in the community. Diur mugan does not disqualify. A supervised beit avot is not automatically a "nursing institution": BL must examine the specific ward (Nat. Labour Court, avl 33417-10-12)
- Needs help with ADL (activities of daily living)
- Passes assessment by Bituach Leumi assessor

### Benefit Levels

| Level | ADL Points | Weekly Hours | With Foreign Worker | Reduced (50%) hours | Max hours to cash | Typical ADL Profile |
|-------|-----------|-------------|---------------------|---------------------|-------------------|---------------------|
| 1 | 2.5-3 | 5.5 | 5.5 | 2.75 (or 4.5 non-personal-care units) | see Level 1 election | Needs help with 1-2 ADL areas |
| 2 | 3.5-4.5 | 10 | 10 | 5 | 4 | Needs help with 3-4 ADL areas |
| 3 | 5-6 | 17 | 14 | half | 4, or 6 with social worker approval (1,488 NIS) | Needs help with most ADL areas |
| 4 | 6.5-7.5 | 21 | 18 | half | 4, or 7 with social worker approval (1,736 NIS) | Needs help with nearly all ADL areas |
| 5 | 8-9 | 26 | 22 | half | 4, or 9 with social worker approval (2,232 NIS) | Dependent in most ADL areas |
| 6 | 9.5-10.5 | 30 | 26 | 15 (13 with foreign worker) | 4, or 10 with social worker approval (2,480 NIS) | Fully dependent in all ADL areas |

Every hours figure above is read from the official per-level pages at
`btl.gov.il/benefits/Long_Term_Care/benefit_level/Pages/level1.aspx` through `level6.aspx`.
Four weekly hours converted to cash is 992 NIS at levels 2-6.

### Level 1 election (unique to level 1)
Level 1 is a choice between four packages, changeable at any time:
1. 5.5 weekly hours of personal home care.
2. Services worth 9 service units that exclude personal home care (day centre, supportive community, absorbent products, panic button, laundry).
3. The whole benefit as cash: 1,705 NIS/month.
4. A mix of cash and services worth 5.5 units (e.g. 3.5 care hours + 620 NIS/month).

Anyone receiving the benefit before 01.11.2018 may keep the old 9 hours 45 minutes per week.

### Day centre conversion
- Level 1: one day = 2 service units, up to 4.5 days/week.
- Level 6: one day = 2.45 service units, up to 5.5 days/week.
- Reduced-benefit recipients: up to 2 days/week.

### Full-time top-up when the caregiver comes through a siud company
The company's share of the post is set by the covered siud hours. The family must top the
post up to a full week at its own expense (a full post is 42 weekly / 182 monthly hours
since 01.04.2018). Budget for this on top of the caregiver cost figures.

### Services Included
- Home caregiver visits
- Adult day care center attendance
- Laundry service
- Absorbent products (incontinence supplies)
- Emergency button monitored service
- Supportive community (kehila tomechet)

### Cash Benefit Option (Kitzva b'Kesef)
Recipients can take part or all of the benefit as monthly cash. Max amounts from 01.04.2026:

| Level | Foreign Worker | Israeli Worker |
|-------|----------------|----------------|
| 1 | 1,705 NIS | 1,705 NIS |
| 2 | 2,480 NIS | 2,480 NIS |
| 3 | 3,472 NIS | 4,216 NIS |
| 4 | 4,464 NIS | 5,208 NIS |
| 5 | 5,456 NIS | 6,448 NIS |
| 6 | 6,448 NIS | 7,440 NIS |

Hour-to-cash conversion (April 2026): Level 1 = 310 NIS/hour; Levels 2-6 = 248 NIS/hour.

Full cash requires documented full-time caregiver (12+ hours/day, 6 days/week, non-family, written contract).

### Means Test (2026)
Income is the claimant's and spouse's average gross income over the three months before the
claim. Thresholds depend on the month the claim is FILED. Couples get the full benefit up to
1.5x the individual threshold and the reduced rate up to 2.25x.

| Filing month | Individual: full | Individual: 50% | Individual: none | Couple: full | Couple: 50% | Couple: none | Per-child add (full / 50%) |
|---|---|---|---|---|---|---|---|
| January 2026 | up to 13,316 | 13,316-19,974 | above 19,974 | up to 19,974 | 19,974-29,961 | above 29,961 | 6,658 / 6,658-9,987 |
| February 2026 | up to 13,467 | 13,467-20,201 | above 20,201 | up to 20,201 | 20,201-30,301 | above 30,301 | 6,734 / 6,734-10,100 |
| March 2026 | up to 13,618 | 13,618-20,427 | above 20,427 | up to 20,427 | 20,427-30,641 | above 30,641 | 6,809 / 6,809-10,214 |
| April-December 2026 | up to 13,769 | 13,769-20,654 | above 20,654 | up to 20,654 | 20,654-30,980 | above 30,980 | 6,885 / 6,885-10,327 |

All amounts in NIS per month. The reduced rate pays half the service units at every level,
or 829 NIS at Level 1 and 3,618 NIS at Level 6 if taken as cash.

**Income NOT counted:** private siudi insurance payouts, mobility allowance, attendance
allowance (shirutim meyuchadim), child allowances, disabled-child allowance, work-injury
payments, and every Holocaust survivor payment (Finance Ministry rente, Claims Conference
Article 2, ZRBG, French and Dutch pensions).

**Expenses deducted from income:** court-ordered alimony; rent paid (including in diur
mugan) up to the amount of rent received; the cost of maintaining a spouse, parent or child
in an institution.

**Both spouses eligible:** each is assessed as an individual on half the joint income.
Same-sex partners count as a couple. No income certificates are required if the claimant
already receives income support, if BL already holds the income data, or if a full income
declaration was filed on a previous claim less than a year ago.

### Exclusions and continuity
- Does NOT cover nursing home costs
- Does NOT cover foreign caregiver salary directly (partial offset via cash option only)
- Stops on genuine placement in a nursing institution or nursing ward. Diur mugan does not
  affect it, and a supervised beit avot is not automatically a nursing institution.
- Hospitalisation: the caregiver continues at the hospital for the first 30 days. After a
  longer stay, or discharge from a nursing institution, eligibility is restored as it was,
  with no condition on the length of the stay.

### Application
- Phone: *6050 or local Bituach Leumi branch
- Service basket choices and the cash election: *2637 (Sun-Thu 08:00-17:00)
- Exceptional full-cash request where home personal care is impossible: *2637 with medical documents
- Assessor visits home to evaluate functional level
- Decision typically within 30-60 days

## Private Nursing Care Insurance (Bituach Siudi)

### Key Rules
- Purchased through kupot cholim. Standalone individual policies are effectively unavailable:
  most insurers stopped marketing them in 2019.
- No statutory age cap, but enrolling by age 49 guarantees max benefits
- Premiums rise sharply with age; above 70 the premium often exceeds expected benefit
- Since December 2023: new joiners only get the basic tier; expanded tier frozen until January 2028
- Free for minors under 18
- Can switch kupot cholim without underwriting or waiting period
- Lower premiums at younger enrollment ages

### What It Provides
Basic tier monthly benefit by age at joining and place of stay (2026, linked to the
November 2023 index):

| Where the insured is | Joined by 49 | Joined 50-59 | Joined 60+ |
|---|---|---|---|
| At home | 5,000 NIS | 4,100 NIS | 3,200 NIS |
| In an institution | 10,000 NIS | 6,500 NIS | 4,500 NIS |

(At-home amounts before 01.01.2024 were 5,500 / 4,500 / 3,500 NIS.)

Terms that decide whether the policy is worth buying:
- Paid for 5 years only, not for life. The frozen expanded tier added 10 more years.
- 60-day waiting period after becoming ADL-dependent.
- The institutional payout is indemnity capped at 80% of what was actually paid.
- Excludes nursing dependency caused by road accidents or work accidents.
- Can be received simultaneously with gimlat siud, and is not counted as income in that
  benefit's means test.

### The Four Kupot Cholim
- Clalit Mushlam/Platinum
- Maccabi Zahav/Sheli
- Meuhedet Adif/Magen
- Leumit Zahav/Kesef

Each offers different siudi plans with different coverage levels and premiums.
